...
describe('listDevices', () => {
  it('returns the devices array on success', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('http://host:8082/api/devices', expect.objectContaining({ signal: undefined }));
...
describe('getStatus', () => {
  it('encodes the serial and returns parsed body', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('http://host:8082/status?serial=S%201', expect.any(Object));
...
describe('subscribeEvents', () => {
  it('parses chunked SSE events and surfaces serials and lifecycle hooks', async () => {
    ...
    const subscriptionPromise = subscribeEvents('http://host:8082', controller.signal, {
  ...
  it('throws inside loop when fetch is synchronously rejected after abort', async () => {
    ...
    const errors: unknown[] = [];
    const promise = subscribeEvents('http://host:8082', outer.signal, {
      onSerial: () => {},
      onError: (e) => {
        errors.push(e);
        outer.abort();
